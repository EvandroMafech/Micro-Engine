...
export default class Spikedball extends AnimatedImage{
    ...
    animate(){
        ...
        this.x = -this.radious*Math.sin(this.amplitude*Math.cos(angularFrequency*this.time + this.phase)) + this.origin.x
        this.y = +this.radious*Math.cos(this.amplitude*Math.cos(angularFrequency*this.time + this.phase)) + this.origin.y
