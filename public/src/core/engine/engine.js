...
export function createGrid(ctx) {
  //cria todas as instancias do grid principal do editor
...
export function createMapBoundaries() {
  //posiciona os tilesets de terreno no canvas para testes
...
export function createBackgroundGrid(ctx) {
  //cria os blocos do background (instancias)
  const tileSize = 64 * 3;
  for (let c = 0; c < lines; c++) {
    for (let l = 0; l < columns; l++) {
      let x = tileSize * l;
...
export function setImageOnBackgroundTiles(image) {
  //posiciona os quadrados de background no canvas
