...
export const activeSelectedImage = {
  //usada para salvar a ultima imagem selecionada pelo cliente
  imageUrl: "",
  imageId: "",
  type: "",
  instance: "",
...
function undoImages() {
  //apaga imagens da tela pelo atalho CTRL-Z
  ...
  if (lastImage == "animated") {
    ...
    const lastImageId =
      animatedImagesArray[animatedImagesArray.length - 1].name;
...
export function createTileId(event) {
  //cria um Id para cada tile do canvas principal
  ...
  const id =
    `l${Math.floor(positionX / tileSize)}` +
    `c${Math.floor(positionY / tileSize)}`;
  return id;
}

export function drawGrid() {
  // desenha o grid do editor
  tileArray.forEach((tile) => tile.draw(ctxGrid));
}

export function clearGrid() {
  //limpa tela do canvas das animações
  ctxGrid.clearRect(0, 0, tileSetCanvas.width, tileSetCanvas.height);
}

export function setTileSetImageOnCanvas(TileId) {
  //posiciona os tilesets de terreno no canvas
...
export function createAnimatedImage(TileId, event) {
  //cria uma imagem animada
  ...
  const { w, line, h, image, frames, id } =
    spriteCoordinates[activeSelectedImage.imageId].location[0];
  let adjustX;
  let adjustY;
  ...
  tileArray.forEach((tile) => {
    if (tile.id == TileId) {
      ...
      if (activeSelectedImage.imageId.includes("fruit")) {
        animatedImage = new Fruit(
          sheetImage,
          x - adjustX,
          y - adjustY,
          activeSelectedImage.imageId,
          frames,
          line,
          w,
          h,
      ...
      allSetIdsArray.push({
        id: activeSelectedImage.imageId,
        type: "animated",
      });
    }
  });
...
export function manageImages(event) {
  //pega a imagem selecionada e joga na função correspondente para criar plotar a imagem
...
drawGrid();
setTimeout(createMapBoundaries, 1000); //por algum motivo se eu chamar direto na sequencia de funções acima nao funciona
animationLoop();
...
window.addEventListener("keydown", (event) => {
  ...
  if (key == "arrowleft") {
    player.MoveAction.left = true;
  }
  if (key == "arrowright") {
    player.MoveAction.right = true;
  }
  if (key == " ") {
  ...
  if (key == "shift") {
    keyboardShortcuts.alignItens = true;
  }
...
window.addEventListener("keyup", (event) => {
  ...
  if (key == "arrowleft") {
    player.MoveAction.left = false;
  }
  if (key == "arrowright") {
    player.MoveAction.right = false;
  }
  if (key == " ") {
    event.preventDefault();
