...
export default class Player {
  ...
  jump() {
    if (
      ...
      !this.playerState.isOnPlatform &&
      this.playerState.keyJumpIsUp
    ) {
  ...
  animate() {
    let position =
      Math.floor(frames / this.staggerFrames) %
    ...
    if (this.MoveAction.left || this.playerState.currentDirection == "left") {
      ...
      this.ctx.translate(
        this.position.x + this.spriteWidth * this.spriteSize,
        this.position.y,
      ...
      this.ctx.drawImage(
        ...
        this.spriteHeight,
        0,
        0, // Ajustado para o sistema de coordenadas transformado
        this.spriteWidth * this.spriteSize,
        this.spriteHeight * this.spriteSize,
      );
  ...
  move() {
    ...
    if (this.MoveAction.left && !this.leftBlocked) {
      ...
      if (gameState.gameRunning == true) {
        if (
          -this.position.x + cameraPosition.initPlayerPositionX >
  ...
  checkCollisionOnTiles() {
    ...
    const collided = tileArray.some((tile) => {
      ...
      const hittingHead =
        playerEdges.top + this.spriteOffset.top * this.spriteSize <=
          tileEdges.bottom &&
        playerEdges.bottom >= tileEdges.bottom &&
  ...
  checkCollisionOnWalls() {
    //verifica se o player está colidindo com as paredes laterais
    const playerEdges = this.calculatePlayerEdges();
    ...
    tileArray.some((Tiles) => {
      ...
      if (
        ...
        if (
          playerEdges.right > tileEdges.right &&
          playerEdges.left < tileEdges.right
        ) {
          this.leftBlocked = true;
        } else if (
          playerEdges.left < tileEdges.left &&
          playerEdges.right > tileEdges.left
        ) {
